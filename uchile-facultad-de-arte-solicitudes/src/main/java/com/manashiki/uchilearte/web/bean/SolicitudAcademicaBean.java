...
@ManagedBean(name = "solicitudAcademicaBean")
@ViewScoped
public class SolicitudAcademicaBean implements Serializable{

	private static final long serialVersionUID = 3409917190126496151L;

	private static final Logger objLog = LoggerFactory.getLogger(SolicitudAcademicaBean.class);
	/*********************************************/
...
//		uchileArte = clienteWsRestUtilidades.listarTipoSolicitudes(new RequestProductoDTO(), ipServer);
	...
	public void handleFileUpload(FileUploadEvent fileUploadEvent){
		...
		try {
			long size = fileUploadEvent.getFile().getSize();
			if(size<20646905){
				GenerarAlmacenamientoArchivos GenerarAlmacenamientoArchivos = new GenerarAlmacenamientoArchivos();

				String filePath = GenerarAlmacenamientoArchivos.generarNombreCarpetaProyectoNombreArchivo("solicitudesAcademicas", upFile.getFileName());


				GenerarAlmacenamientoArchivos.almacenarObjectFile(upFile, filePath, upFile.getContents());

				solicitudAcademicaDTO.setNombreArchivo(upFile.getFileName());
				...
				archivoSolicitudDTO.setNombreArchivoSolicitud(upFile.getFileName());
...
//				uchileArte = clienteWsRestUtilidades.crearArchivoSolicitud(requestProductoDTO, ipServer);
				...
				FacesMessage message = new FacesMessage("Archivo ", fileUploadEvent.getFile().getFileName() + " ha sido añadido a la solicitud.");
				...
				FacesMessage message = new FacesMessage("Archivo ", fileUploadEvent.getFile().getFileName() + " el archivo ha excedido el tamaño maximo 20MB.");
			...
			if(solicitudAcademicaDTO.getApellidoPaternoPersonaSolicitudAcademica()!=null && !solicitudAcademicaDTO.getApellidoPaternoPersonaSolicitudAcademica().equals("")){
				if(solicitudAcademicaDTO.getApellidoMaternoPersonaSolicitudAcademica()!=null && !solicitudAcademicaDTO.getApellidoMaternoPersonaSolicitudAcademica().equals("")){
					if(solicitudAcademicaDTO.getRutPersonaSolicitudAcademica()!=null && !solicitudAcademicaDTO.getRutPersonaSolicitudAcademica().equals("")){
									...
									if(ValidacionPatrones.validarPatronEmail(solicitudAcademicaDTO.getMailMember())){	
										if(solicitudAcademicaDTO.getAnhoIngreso()!=null && !solicitudAcademicaDTO.getAnhoIngreso().equals("")){
											if(solicitudAcademicaDTO.getIdTipoSolicitud()!=0){
												...
												FacesMessage msg=new FacesMessage(FacesMessage.SEVERITY_ERROR,"ERROR", "No se seleccionó tipo de solicitud del alumno");
												FacesContext.getCurrentInstance().addMessage(null, msg);
											}
										}else{
											FacesMessage msg=new FacesMessage(FacesMessage.SEVERITY_ERROR,"ERROR", "No se seleccionó año de ingreso del alumno");
											FacesContext.getCurrentInstance().addMessage(null, msg);
										}
									}else{
						...
						FacesMessage msg=new FacesMessage(FacesMessage.SEVERITY_ERROR,"ERROR", "No se ingreso rut del alumno");
						FacesContext.getCurrentInstance().addMessage(null, msg);
					}
				}else{
					FacesMessage msg=new FacesMessage(FacesMessage.SEVERITY_ERROR,"ERROR", "No se ingreso apellido materno del alumno");
					FacesContext.getCurrentInstance().addMessage(null, msg);
				}
			}else{
				FacesMessage msg=new FacesMessage(FacesMessage.SEVERITY_ERROR,"ERROR", "No se ingreso apellido paterno del alumno");
				FacesContext.getCurrentInstance().addMessage(null, msg);
			}
		}else{
			FacesMessage msg=new FacesMessage(FacesMessage.SEVERITY_ERROR,"ERROR", "No se ingreso nombre del alumno");
			FacesContext.getCurrentInstance().addMessage(null, msg);
		}
	 ...
	 * no lanza Excepciones.
	 */
	public void almacenarSolicitudAcademicaPagoOffline() throws IOException {
